...
export default (parent, text) => (sketch) => {
  ...
  const fillDots = (width, height) => {
    dots = [];
    let [firstWord, secondWord, thirdWord] = text.split(' ') 

    if (width > height) {

      firstWordPoints = font.textToPoints(`${firstWord} ${secondWord}`, width * 0.02, height * 0.33, width * 0.11)
      secondWordPoints = font.textToPoints(thirdWord, width * 0.03, height * 0.83, width * 0.27);
    } else {
